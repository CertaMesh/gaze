...
| MUT-NEGATIVE-COVERAGE-COUNT | `c.1.get("negative_compared") != c.1.get("negative")` → `false` (receipt incomplete match; leading newline and 16 spaces) | `r2_negative_predicate_coverage` | `negative-incomplete-count` |
| MUT-NEGATIVE-COVERAGE-GATE | `c.1.get("negative_compared") != c.1.get("negative")` → `false` (false_positive_negative_control gate; leading newline and 8 spaces) | `r2_negative_predicate_coverage` | `negative-coverage-gate` |
...
| MUT-RAW-COVERAGE-GATE | `c.1.get("raw_compared").copied().unwrap_or(0) != restores` → `false` (egress_integrity_analogues gate; leading newline and 8 spaces) | `r2_mixed_raw_comparison_coverage` | `raw-coverage-gate` |
