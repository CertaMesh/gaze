...
| Runtime info | Cold start ms ↓ (goal no regression) | Warm p50 ms ↓ (goal no regression) | Warm p95 ms ↓ (goal no regression) | Correctness ↔ (goal exact) |
| --- | ---: | ---: | ---: | --- |
...
| Build info | Feature set info | Bytes ↓ (goal lower) | MiB ↓ (goal lower) |
| --- | --- | ---: | ---: |
