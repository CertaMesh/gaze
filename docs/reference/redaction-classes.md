...
| `core, core-extended` | `ip.v6` | `regex` | Full, compressed (including bare `::`), and IPv4-embedded IPv6 textual forms at every locale. The word guard excludes an address adjacent to an identifier character, so Rust and C++ `::` paths survive; an explicit `Address:`, `Adresse:`, `IP:`, `IPv6:`, `host:` or `addr:` cue admits a glued address after full IPv6 parsing (todos #3710 and #3762, superseding #2402). `_2001:db8::1` remains outside the cue rule. A standalone path whose segments are all short hex words and which has no surrounding context (`a::b`, `abc::def`) is still an address | `custom:ip_address` | `global` | `ipv6_parse` | `none` | `safe_default` | yes | 0.70 | 80 |
