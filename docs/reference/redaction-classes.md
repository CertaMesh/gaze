...
| `core, core-extended` | `card.structural` | `regex` | a digit run with optional spaces or dashes; the recognizer finds the Luhn-valid 13 to 19 digit card inside it (the old 13-19 window, else the longest group-aligned card layout: compact, 4-4-4-4, 4-4-4-4-3, 4-6-5, 4-6-4), so a touching CVV, expiry or number does not hide it | `custom:credit_card` | `global` | `luhn` | `none` | `safe_default` | yes | 0.70 | 80 |
