...
| `core, core-extended` | `ip.v6` | `regex` | Full, compressed (including bare `::`), and IPv4-embedded IPv6 textual forms at every locale, guarded by a word boundary: a candidate that begins right after an identifier character or ends right before one does not fire, so Rust and C++ `::` paths survive (todo #3710, superseding #2402). A standalone path whose segments are all short hex words and which has no surrounding context (`a::b`, `abc::def`) is still an address | `custom:ip_address` | `global` | `ipv6_parse` | `none` | `safe_default` | yes | 0.70 | 80 |
