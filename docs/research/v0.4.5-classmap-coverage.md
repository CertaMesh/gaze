...
|-----|---------|-----------|----------------------------------------|-------|-----------|
| 1 | Context JSON parse | `crates/gaze/src/context.rs::Context::from_json_str` | Yes, by parsing `class_map` values into `PiiClass`. | Parse-time only: unknown classes fail closed with `ContextError::UnknownClass`. | N/A for action coverage because no recognizer is built in this layer; guarded by `crates/gaze/src/context.rs::tests::parses_typed_context_envelope` and `ContextError::UnknownClass` parsing. |
