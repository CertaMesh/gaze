...
class PrivateEvaluator:
    ...
    def add(self, arm, key, record):
        ...
        for f in fields(record):
            if f.name not in ('outcome','pii_class','region','restore_exact','restore_decision_success'):
    ...
    def aggregate(self, arm):
        self.finalize()
        result = {metric:sum(getattr(r, field) for r in self._records[arm].values()) for metric,field in METRIC_FIELDS.items()}
        result['unknown_egress_lower_bound_cases'] = sum(r.outcome == 'UNKNOWN_EGRESS' and (r.gold_bytes_surviving > 0 or r.gold_occurrences_surviving > 0 or r.gold_occurrences_partially_surviving > 0) for r in self._records[arm].values())
        return result
    ...
    def export_receipt(self, template, custody, *, arm='candidate', declaration=None, repo_root=None, attestation_probe=None):
        self.finalize()
        ep.require(type(custody) is dict and custody.get('scope') == 'synthetic_fixture_only' and tuple(custody.get('membership_order', ())) == self.inventory.keys(), 'membership_order_proof_failed')
        ep.require(arm in ep.ARM_IDS, 'value_out_of_vocabulary')
        r = copy.deepcopy(template)
        r.update(arm_id=arm, planned_case_count=len(self.inventory), outcomes=self.outcomes(arm), analysis_declaration=declaration)
        ...
        r['derivations'] = {k:'egress_reconstructed' if k in r['counts'] else 'not_measured' for k in ep.METRIC_IDS}
        ...
        for k in ('receipt_path_allowlist','vocabulary_closure','stamped_field_separation','outcome_identities','planned_inventory_reconciliation','rejection_is_not_protection','unknown_egress_lower_bound','declaration_gating','claim_scope_present'):
        ...
        r['intervals'] = {m:self.paired_interval(m, declaration) for m in METRIC_FIELDS} if arm == 'candidate' else {}
        r['error_codes'] = {}
