...
- **Ambiguity side-channel (v0.7.x):** `RedactionEntry` can carry optional `ValidatorFailReason` and `AmbiguityRecord` metadata. `SqliteLogger` persists the bundled four-column migration (`validator_fail_reason`, `ambiguity_record`, `collision_family`, `collision_variant`) and CLI audit queries can filter ambiguity/collision metadata. Contract: [`docs/architecture/ambiguity-side-channel.md`](docs/architecture/ambiguity-side-channel.md).
- **Validator-veto (v0.7.x):** validator-backed recognizer failures are rejected before conflict resolution and logged as loser-only audit rows with `decided_by: ValidatorVeto` plus typed `validator_fail_reason`. Existing `ConflictTier::Validator` remains the same-class containment tie-breaker. Contract: [`docs/architecture/validator-veto.md`](docs/architecture/validator-veto.md).
