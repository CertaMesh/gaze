//! Shared trybuild support for this crate's compile-fail drivers.
//!
//! Both `tool_ctx_seal.rs` (the `ToolCtx` seal) and `tier_isolation.rs` (the
//! agent/operator tier boundary) assert against `.stderr` snapshots, which are
//! only meaningful under the compiler the workspace pins. Setting
//! `RUSTUP_TOOLCHAIN` alone does not bind Cargo's children, so this guard fails
//! loudly rather than letting a snapshot mismatch read as a real regression.

use std::fs;
use std::path::{Path, PathBuf};
use std::process::Command;

pub fn assert_trybuild_rustc_matches_workspace() {
