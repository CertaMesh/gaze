...
//! header set containing only the canonical transformed-body `content-type`, mirroring the strict header
...
async fn bare_upstream(
    ...
    (
        StatusCode::OK,
        HeaderMap::new(),
        axum::body::Body::from(serde_json::to_vec(&upstream_response_body(&body)).unwrap()),
    )
}
...
async fn parameterized_upstream(Json(body): Json<Value>) -> impl IntoResponse {
    axum::response::Response::builder()
        .header(
            "content-type",
            "application/json; owner=alice@example.invalid",
        )
        .body(axum::body::Body::from(
            serde_json::to_vec(&upstream_response_body(&body)).unwrap(),
        ))
        .unwrap()
}

#[tokio::test]
async fn legacy_response_does_not_forward_pii_in_content_type_parameters() {
    let upstream = spawn_upstream_with(post(parameterized_upstream)).await;
    let proxy = spawn_proxy(Arc::new(OpenAiAdapter::new(upstream.base_url.clone()))).await;
    let response = post_downstream(&proxy, "/v1/completions", &openai_request()).await;
    assert_eq!(response.status(), StatusCode::OK);
    assert_content_type_preserved(response.headers(), "application/json");
    assert!(response.headers().values().all(|value| !value
        .as_bytes()
        .windows(EMAIL.len())
        .any(|window| window == EMAIL.as_bytes())));
    assert_openai_body_round_trips(&response.json().await.unwrap());
}
