...
use super::*;
use gaze::experimental_benchmark_baseline_lock::{
    BenchmarkBaselineLock, BenchmarkLockPolicy, Disposition,
};
...
fn bind_assembled(
    ...
    let pipeline =
        gaze_assembly::build_pipeline(&ruleless, &context, &[rulepack], &locales, threshold)?;
...
impl Producer {
    pub(super) fn build() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(Self {
            baseline,
            detector,
            audit: RefCell::new(Audit::new(Box::new(audit))),
        })
    }
    ...
    fn evidence(
        ...
        if let Some(error) = audit.failure {
        ...
        let status = match event {
            ...
            AuditEvent::BatchComplete(dispositions) => {
                ...
                let counts = dispositions
                    .iter()
                    .fold([0usize; 3], |mut counts, disposition| {
    ...
    fn clean(&self, ordinal: u64, request: Request) -> Result<Outcome, Box<dyn std::error::Error>> {
        let locales = request
            .locale_chain
            .iter()
            .map(|s| LocaleTag::parse(s))
            .collect::<Result<Vec<_>, _>>()?;
        let prefix = session_hex_for_fixture(&request.fixture_id);
        let start = Instant::now();
        let result =
            self.baseline
                .clean_redact_text(&request.text, prefix, &locales, &self.detector);
        ...
        observe_clean(
            ...
            RestoreSource::Locked(&self.baseline),
            request.fixture_id,
            request.text,
            output.session,
            output.text,
            output.manifest,
            gaze::LeakReport::default(),
            output.trace,
            locales,
            clean_ms,
        )
...
"##
    ...
    fn request(text: &str) -> Request {
        Request {
            fixture_id: "synthetic-lock-proof".into(),
            locale_chain: vec!["de-CH".into(), "en-US".into(), "de-CH".into()],
            text: text.into(),
        }
    }
    fn semantic_fields(outcome: Outcome) -> Value {
        let Outcome::Success(response) = outcome else {
            panic!("synthetic request refused");
        };
    ...
    fn synthetic_producer(detector: RedactDetector, audit: Box<dyn Write>) -> Producer {
        Producer {
            baseline: assemble(None, false).unwrap(),
            detector,
            audit: RefCell::new(Audit::new(audit)),
        }
    }
    #[test]
    fn embedded_assembly_empty_supplement_matches_all_observed_fields() {
        ...
        for (index, raw) in [
            ...
            "Email: ａｌｉｃｅ＠ｅｘａｍｐｌｅ．ｉｎｖａｌｉｄ",
            "Dr. Schmidt visits Example Research GmbH",
        ]
        .iter()
        .enumerate()
        {
            let reference = semantic_fields(
                handle_request(BenchConfig::Pass2Ner, &ordinary, request(raw)).unwrap(),
            );
            let candidate =
                semantic_fields(producer.handle(index as u64 + 1, request(raw)).unwrap());
    ...
    fn original_three_arms_keep_ordinary_dispatch_and_direct_pipeline_results() {
        for config in [
            ...
            assert!(
                !config.uses_baseline_lock(),
                "production build_producer must select the ordinary recipe"
            );
            ...
            let (_dir, detector) =
                bridge(json!([{"start":0,"end":2,"label":"GIVEN_NAME","score":0.1}]));
            // Frozen original construction routes, with NER intentionally absent in this plumbing proof.
            let pipeline = match config {
                BenchConfig::Pass2Ner => {
                    gaze_assembly::build_pipeline(&policy, &context, &[rulepack], &locales, None)
                        .unwrap()
                }
                BenchConfig::Pass2NerRedact => gaze_assembly::build_pipeline_with_detector(
                    &policy,
                    &context,
                    &[rulepack],
                    &locales,
                ...
                BenchConfig::Pass2NerRedactSemantic => {
                    let audit =
                        semantic_admission::AuditSink::new(&_dir.path().join("semantic.jsonl"))
                            .unwrap();
                    let recognizer =
                        semantic_admission::SemanticRecognizer::with_audit(detector, audit);
                    gaze_assembly::build_pipeline_with_recognizer(
                        &policy,
                        &context,
                        &[rulepack],
                        &locales,
            ...
            let session = Session::new_with_session_hex_for_tests(
                Scope::Ephemeral,
                session_hex_for_fixture("synthetic-lock-proof"),
            )
            .unwrap();
            ...
            assert_eq!(
                actual["manifest_spans"],
                serde_json::to_value(serialize_manifest(manifest)).unwrap()
            );
            assert_eq!(
                actual["final_protection_trace"],
                serde_json::to_value(serialize_final_protection_trace(trace)).unwrap()
            );
            assert_eq!(
                actual["initial_safety_net_stats"],
                serde_json::to_value(SafetyNetStats::from(&report.stats)).unwrap()
            );
    ...
    fn audit_begin_batch_and_finalization_failures_never_release_success() {
        for (fail_at, fail_flush_at) in [
            ...
            let producer = synthetic_producer(
                detector,
                Box::new(AuditProbe {
                    writes: writes.clone(),
                    fail_at,
            ...
            if fail_at == usize::MAX && fail_flush_at == usize::MAX {
                ...
                assert_eq!(
                    records
                        ...
                        .collect::<Vec<_>>(),
                    ["request_begin", "batch_complete", "request_success"]
                );
                ...
                assert!(producer
                    .handle(2, request("alice@example.invalid"))
                    .is_err());
                assert_eq!(
                    writes.borrow().len(),
                    count,
                    "audit failure must remain closed"
                );
            }
        }
    }
    ...
    fn lifecycle_records_omit_counts_and_only_complete_batches_report_zero() {
        ...
        for lifecycle in [
            ...
            producer
                .evidence(1, AuditEvent::Lifecycle(lifecycle))
                .unwrap();
        }
        producer
            .evidence(1, AuditEvent::BatchComplete(&[]))
            .unwrap();
        producer
            .evidence(
                2,
                AuditEvent::BatchComplete(&[Disposition::Admitted, Disposition::BaselineOverlap]),
            )
            .unwrap();
        let records = writes
            .borrow()
        ...
        for lifecycle in &records[..4] {
            for key in ["admitted", "baseline_overlap", "supplemental_overlap"] {
                assert!(
                    lifecycle.get(key).is_none(),
                    "unknown batch must not report zero"
                );
            }
        }
        for key in ["admitted", "baseline_overlap", "supplemental_overlap"] {
            assert_eq!(records[4][key], 0);
        }
    ...
    fn total_audit_cap_including_final_record_refuses_without_more_writes() {
        ...
        assert!(matches!(
            producer.handle(1, request("alice@example.invalid")),
        ...
        for allowed in [lengths[0] - 1, lengths[0] + lengths[1] - 1, total - 1] {
            ...
            assert!(producer
                .handle(1, request("alice@example.invalid"))
                .is_err());
    ...
    fn candidate_preflights_context_policy_floor_and_missing_ner() {
        ...
        assert!(build_pipeline_internal(
            BenchConfig::Pass2NerRedactBaselineLock,
        ...
        for context_field in 0..3 {
            ...
            match context_field {
                0 => {
                    context.fields.insert("synthetic".into(), json!(true));
                }
                1 => {
                    context.class_map.insert("synthetic".into(), PiiClass::Name);
                }
                _ => {
                    context.dictionaries.insert(
                        "synthetic".into(),
                        gaze::ContextDictionary {
                            terms: vec!["synthetic".into()],
                            case_sensitive: true,
                        },
                    );
                }
