...
mod tests {
    ...
    fn locked_restore_uses_shared_classifier_without_provider() {
        ...
        let locked = BenchmarkLockPolicy::try_from(Vec::<RuleSpec>::new())
            ...
            .bind(baseline)
            .unwrap();
        let source = RestoreSource::Locked(&locked);
        ...
        let (restored, telemetry) = source.restore_with_telemetry(&session, &input).unwrap();
        ...
        for unknown in [
            ...
            let (_, telemetry) = source.restore_with_telemetry(&session, &input).unwrap();
            assert_eq!(
