...
#[test]
fn root_policy_example_loads_under_current_policy_loader() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let policy_path = manifest_dir.join("../..").join("policy.example.toml");

    let policy = Policy::load(&policy_path).expect("root policy.example.toml must parse");
