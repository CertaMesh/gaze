...
|---|---:|---:|---:|---:|---:|
| **v0.15.1 default: `gaze setup` policy (rules + NER + Nym net)** | 0 | **19,556 (15.0%)** | 19,556 (15.0%) | 30,073 | 100.0% |
| v0.15.0 default: the same policy | 0 | 19,556 (15.0%) | 19,556 (15.0%) | 30,073 | 100.0% |
...
Neither default refused a document, so all 2,910 processed documents are also the common set, and every document restores exactly. v0.15.1 scores the same as v0.15.0 here: its card fix covers cards that pass the Luhn check, and most of this corpus's card numbers do not. Against v0.14.0's default (rules + NER + the since-removed Kiji net: 25,179 B leaked, 168,276 false-positive bytes), the `gaze setup` default leaks 22% fewer PII bytes with 82% fewer false-positive bytes; the [benchmark history](docs/reference/benchmarks/README.md#release-history) has every measured arm. All rows use scored-label contract v1, which scores every corpus label; under contract v2 (PASSWORD and SECURITYTOKEN out of contract, gold 123,621 B) the same v0.15.1 run leaks 13,319 B (10.77%).
