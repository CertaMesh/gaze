...
- **Long-lived stdio runtime for repeated redaction.** `gaze daemon` keeps one pipeline and model load hot, then serves JSON-per-line requests with per-`session_id` manifest isolation. It avoids binary/model cold starts on every agent turn, exits gracefully on SIGTERM, and evicts sessions by LRU or idle timeout. Adopter quickstart: [`docs/getting-started/daemon-adapter.md`](docs/getting-started/daemon-adapter.md). Full contract: [`docs/architecture/daemon-mode.md`](docs/architecture/daemon-mode.md).
